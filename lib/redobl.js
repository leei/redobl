...
Redobl.prototype = {
  ...
  generatingID: function(callback) {
    if (this._generating) {
      ...
      this.on('after_id', function () { callback.call(that); });
    } else if (this._id) {
  ...
  destroy: function(callback) {
    if (this.id) {
      var assocs = this.constructor.associations;
      var keys = [];
      for (var aname in assocs) {
        keys.push(this[aname].redis_key);
      }
      keys.push(callback);
      this.del.apply(this, keys);
    }
  },
